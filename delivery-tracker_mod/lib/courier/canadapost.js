...
var parser = {
  trace: function (data) {
    ...
    var result = {
      ...
      expectedDelivery: data.expectedDlvryDateTime.revisedDate ? data.expectedDlvryDateTime.revisedDate : data.expectedDlvryDateTime.dlvryDate,
      expectedDlvryWindow: data.expectedDlvryWindow,
      // error: tracker.error(e.message)
    }
    ...
    for (var i = 0; i < data.events.length; i++) {
      ...
      var checkpoint = {
        courier: courier,
        location: (item.locationAddr && item.locationAddr.countryNmEn) ? item.locationAddr.countryNmEn + ' ' + item.locationAddr.city : '',
        message: item.descEn,
